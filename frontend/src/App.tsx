...
const RoleRedirect: React.FC = () => {
  const tokenString = localStorage.getItem("token");
  if (!tokenString) return <Navigate to="/login" replace />;

  const managerId = localStorage.getItem("managerId");
  const agentId = localStorage.getItem("agentId");

  // ✅ manager wins (covers both case automatically)
  if (managerId) return <Navigate to="/manager/team-view" replace />;

  // ✅ agent only
  if (agentId) return <Navigate to="/admin/agent/schedule" replace />;
