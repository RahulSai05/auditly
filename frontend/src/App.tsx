...
export default function App(): JSX.Element {
  ...
  const ProtectedRoute = ({ children, requiredRoles = [] }: ProtectedRouteProps) => {
    ...
    if (requiredRoles.length > 0 && authState.userData) {
      const hasRequiredRole = requiredRoles.some(role => 
        authState.userData?.["User Type"]?.includes(role)
      );
    ...
    <>
      ...
      <Routes>
        ...
        <Route element={
          ...
          {/* Reports User Routes */}
          <Route path="/admin/reports/items" element={
            <ProtectedRoute requiredRoles={["reports_user"]}>
              <DashboardTables />
            </ProtectedRoute>
          } />
          <Route path="/admin/reports/customer-serials" element={
          ...
          <Route path="/admin/dashboard" element={
            <ProtectedRoute requiredRoles={["inpection_user"]}>
              <Dashboard />
            </ProtectedRoute>
          } />
          <Route path="/admin/settings/Item-master-upload" element={
