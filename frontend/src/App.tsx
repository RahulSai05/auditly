...
const InspectionRoute: React.FC<ProtectedRouteProps> = ({ children }) => {
  const userData = JSON.parse(localStorage.getItem("token") || "null");
  const isInspectionUser = userData?.["User Type"]?.includes("inpection_user");
...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    if (authRoutes.includes(location.pathname)) {
      setCheckingUserValidity(false); // skip validation
    }
  }, [location.pathname]);
  ...
  useEffect(() => {
    const validate = async () => {
      ...
      try {
        ...
        const permsOk = required.every(role =>
          role === "admin" ? userExists.is_admin :
          role === "reports_user" ? userExists.is_reports_user :
          role === "inpection_user" ? userExists.is_inpection_user : false
        );
  ...
  // Prevent flashing or stuck screen
  if (checkingUserValidity && !authRoutes.includes(location.pathname)) {
    return <LoadingScreen />;
  }
