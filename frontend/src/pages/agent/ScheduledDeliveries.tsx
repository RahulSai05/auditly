...
//   const fetchOptimizedRoute = async (userLocation, addresses, routeMode) => {
//     try {
...
//       const mappedOrders = result.ordered_addresses.map((addr) =>
//         addressMap[addr.toLowerCase().trim()]
//       ).filter(Boolean);
  
//       console.log("✅ Mapped orders from addresses:", mappedOrders);
  
//       setRouteInfo({ ...result, ordered_orders: mappedOrders });
//     } catch (err) {
//       console.error("❌ Failed to fetch optimized route:", err);
//       setRouteError("Unable to get optimized route.");
//     }
//   };
...
//             order.shipped_to_street && order.shipped_to_city &&
//             order.shipped_to_state && order.shipped_to_zip && order.shipped_to_country
//           );
...
//             const full = `${order.shipped_to_street}, ${order.shipped_to_city}, ${order.shipped_to_state}, ${order.shipped_to_zip}, ${order.shipped_to_country}`;
//             addresses.push(full);
//             addressMap[full.toLowerCase().trim()] = order;
//           });
  
//           setAddressMap(addressMap);
  
//           console.log("📫 Sending to fetchOptimizedRoute:", {
//             userLocation,
//             addresses,
//             routeMode,
//           });
...
//         } catch (err) {
//           console.error("❌ Error inside geolocation callback:", err);
//           setRouteError("Something went wrong.");
//         }
//       },
//       (error) => {
//         console.error("❌ Geolocation error:", error);
//         setRouteError("Location permission denied or failed.");
//       },
//       {
...
//     return (
//       <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 sm:p-8 flex justify-center items-center">
//         <motion.div
...
//           </motion.div>
//           <h3 className="text-xl font-semibold text-slate-700 mb-2">Loading Your Deliveries</h3>
//           <p className="text-slate-500">Please wait while we fetch your scheduled deliveries...</p>
//         </motion.div>
//       </div>
//     );
//   }
...
//                 className="w-24 h-24 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl hover:shadow-blue-300 transition-all duration-300"
//               >
...
//                 className="text-3xl sm:text-5xl font-bold text-slate-900 mb-4 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-700"
//               >
//                 My Deliveries
//               </motion.h1>
//               <motion.p
...
//               >
//                 Manage your delivery schedule and optimize your routes
//               </motion.p>
//             </div>
...
//                       <p className="text-sm text-slate-600">Find the fastest delivery route based on your location</p>
//                     </div>
//                   </div>
//                   <motion.button
...
//                     <span>You need at least 2 deliveries to optimize your route</span>
//                   </div>
//                 )}
//               </div>
//             </div>
...
//                       <p className="text-sm text-green-700">Your most efficient delivery path is ready</p>
//                     </div>
//                   </div>
//                   <button 
...
//                         Start Navigation
//                       </motion.button>
//                       <p className="text-sm text-green-700 mt-2">Navigate to your first delivery stop</p>
//                     </div>
//                   )}
...
//                   {orders.length === 0
//                     ? "You don't have any scheduled deliveries at the moment. New deliveries will appear here when assigned."
...
//                     {filteredOrders.length} Delivery{filteredOrders.length !== 1 ? 's' : ''} Found
//                   </h2>
...
interface RouteResponse {
  ...
  total_duration_minutes: number;
  route_summary: string;
  ordered_orders?: Order[]; // mapped orders (frontend-only)
}
...
const ScheduledDeliveries: React.FC = () => {
  ...
  // IMPORTANT: keys will be normalized addresses
  const [addressMap, setAddressMap] = useState<Record<string, Order>>({});
  const [deliveryTime, setDeliveryTime] = useState<number>(0);
  ...
  // ✅ address normalizer (prevents mismatch issues)
  ...
  // ✅ UPDATED: fetchOrders matches backend "return-orders" shape
  const fetchOrders = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await fetch(
        `/api/agent/sales-orders/${agentId}?organization=${org}`
      );
      ...
      setDeliveryTime(data.total_delivery_time || 0);

      // optional: reset route info when reloading orders (prevents stale route)
      setRouteInfo(null);
      setRouteError(null);
      setAddressMap({});
    } catch (err) {
  ...
  // ✅ UPDATED: improved mapping ordered_addresses -> orders
  const fetchOptimizedRoute = async (
    userLocation: string,
    addresses: string[],
    mode: "FIFO" | "LIFO",
    mapSnapshot: Record<string, Order> // pass snapshot so we don't race setState
  ) => {
    try {
      ...
      const mappedOrders = (result.ordered_addresses || [])
        .map((addr: string) => {
          const normalized = normalize(addr);

          // map keys are already normalized
          const matchEntry = Object.entries(mapSnapshot).find(([key]) => {
      ...
      console.error("❌ Failed to fetch optimized route:", err);
      setRouteError(
        err instanceof Error ? err.message : "Unable to get optimized route."
      );
    } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          // validate orders with required address fields
          ...
          validOrders.forEach((order) => {
            ...
            freshMap[normalize(full)] = order;
          });

          setAddressMap(freshMap);

          await fetchOptimizedRoute(userLocation, addresses, routeMode, freshMap);
        } catch (err) {
  ...
  const filteredOrders = orders
    ...
    .sort((a, b) => {
      if (sortBy === "date") {
        return (
          new Date(b.date_shipped).getTime() - new Date(a.date_shipped).getTime()
        );
      }
      // TODO: implement city sort if needed
      return 0;
    });
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 sm:p-8 flex justify-center items-center">
        ...
        >
          ...
          <p className="text-slate-500">
            Please wait while we fetch your scheduled deliveries...
          </p>
        </motion.div>
      </div>
    );
          ...
          <div className="p-6 sm:p-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        <strong>FIFO</strong>: Start from HUB/DC Address
                        <br />
                        <strong>LIFO</strong>: End at HUB/DC Address
                      </p>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-center">
                    <select
                      value={routeMode}
                      onChange={(e) =>
                        setRouteMode(e.target.value as "FIFO" | "LIFO")
                      }
                      className="px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm font-medium"
                    >
                      <option value="FIFO">Start from HUB/DC Address (FIFO)</option>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-700">
                        Your most efficient delivery path is ready
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-2xl border border-green-100 shadow-sm">
                      ...
                      <p className="text-xl font-bold text-slate-900">
                        {routeInfo.route_summary || "Optimized"}
                      </p>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-green-100 shadow-sm">
                      ...
                      <p className="text-xl font-bold text-slate-900">
                        {Math.floor(routeInfo.total_duration_minutes / 60)}h{" "}
                        {Math.round(routeInfo.total_duration_minutes % 60)}m
                      </p>
                    </div>
                  </div>
                  ...
                  {routeInfo?.ordered_addresses?.[1] && (
                    <div className="mb-8 text-center">
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() =>
                          openGoogleMapsNavigation(routeInfo.ordered_addresses[1])
                        }
                        className="flex items-center gap-3 px-8 py-4 rounded-2xl bg-gradient-to-r from-green-600 to-emerald-600 text-white hover:from-green-700 hover:to-emerald-700 transition-all shadow-lg hover:shadow-green-300 mx-auto font-semibold text-lg"
                      >
                        <Navigation className="w-6 h-6" />
                      ...
                      <p className="text-sm text-green-700 mt-2">
                        Navigate to your first delivery stop
                      </p>
                    </div>
                  )}
                  ...
                  <div className="border-t border-green-200 pt-6">
                    ...
                    <div className="space-y-4">
                      {routeInfo.ordered_addresses.map((address, index) => {
                        ...
                        // ✅ FIXED: use same normalize logic as route mapping
                        const matchedOrderEntry = Object.entries(addressMap).find(
                          ([key]) =>
                            normalizedAddress.includes(key) ||
                            key.includes(normalizedAddress)
                        );

                        const order = matchedOrderEntry
                          ? matchedOrderEntry[1]
                          : undefined;
            ...
            {filteredOrders.length === 0 ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-slate-900">
                    {filteredOrders.length} Delivery
                ...
                {/* NOTE: the rest of your per-order cards JSX stays exactly as you had it.
                    You can paste your existing expanded card JSX below unchanged.
                    (No functional changes needed beyond fetchOrders + route mapping + delivery sequence mapping.)
                */}
                {filteredOrders
                  .filter((order) => order.item_id && order.shipped_to_person)
                  .map((order, index) => (
