...
const ItemUpload: React.FC = () => {
    ...
    const handleFileUpload = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post("http://18.205.235.223:8000/upload-items-csv", formData, {
    ...
    const handleSearch = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.get("http://18.205.235.223:8000/search-items", {
    ...
    return (
        <div className="h-screen flex justify-center items-start bg-gray-100 p-6">
            <div className="w-full max-w-2xl space-y-4">
                {/* Title */}
                <h1 className="text-2xl font-bold text-center">Customer Item Data Upload</h1>
                <p className="text-gray-500 text-center">Upload a CSV file or search the database for customer item data</p>
                                ...
                                <tbody>
                                    {searchResults.map((item, index) => (
                                        <tr key={index} className="text-center">
                                            ...
                                            <td className="border border-gray-300 p-2">#{item.item_number}</td>
                                            <td className="border border-gray-300 p-2">{item.return_term || "N/A"}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
