...
const ConnectionsList = ({ connections, onDelete, isLoading }) => {
  const activeConnections = connections.filter(conn => conn.connection_status === 'Active');
  const inactiveConnections = connections.filter(conn => conn.connection_status !== 'Active');
  ...
  if (connections.length === 0) {
    return (
      <div className="text-center py-8 px-4">
        ...
        <h3 className="text-gray-900 font-medium mb-1">No Connections Found</h3>
        <p className="text-sm text-gray-500">Click the connect button to add your first connection.</p>
      </div>
    );
  }
        ...
        <div>
          ...
          <div className="grid gap-3">
            {activeConnections.map((connection, index) => (
              <motion.div
                key={connection.power_bi_email}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.05 }}
                className="relative group bg-gradient-to-r from-green-50 to-transparent border border-green-100 rounded-xl p-4 hover:shadow-md transition-all duration-300"
              >
                <div className="absolute top-0 right-0 w-24 h-full bg-gradient-to-l from-green-50/50 to-transparent rounded-r-xl" />
                <div className="relative flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-medium text-gray-900 truncate">
                        {connection.power_bi_username || 'Unknown User'}
                      </h4>
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-500">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="truncate">{connection.power_bi_email}</span>
                      </div>
                      <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={() => onDelete(connection.power_bi_email)}
                    className="opacity-0 group-hover:opacity-100 flex-shrink-0 p-2 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 transition-all duration-200"
                  >
                    <Trash2 className="w-5 h-5" />
                  </button>
                </div>
      ...
      {inactiveConnections.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-medium text-gray-900">Inactive Connections</h3>
          ...
          <div className="grid gap-3">
            {inactiveConnections.map((connection, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.05 }}
                className="relative group bg-gradient-to-r from-amber-50 to-transparent border border-amber-100 rounded-xl p-4 hover:shadow-md transition-all duration-300"
              >
                <div className="absolute top-0 right-0 w-24 h-full bg-gradient-to-l from-amber-50/50 to-transparent rounded-r-xl" />
                <div className="relative flex items-start gap-4">
