...
const Inbound: React.FC = () => {
    ...
    try {
      ...
      checkIntervalRef.current = setInterval(() => {
        try {
          ...
          } else if (popupWindowRef.current?.location.href.includes('/admin/settings/connectors/inbound')) {
            // Popup was redirected back to our app
            clearInterval(checkIntervalRef.current as NodeJS.Timeout);
            popupWindowRef.current.close();
            setIsPopupOpen(false);
          }
        } catch (e) {
          // Cross-origin error means we can't check location, but popup is still open
        }
      }, 500);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 relative ${isPopupOpen ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  {source.authEndpoint && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => handleAuthClick(source)}
                      disabled={loading[source.id] || isPopupOpen}
                      className={`px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white text-sm font-medium rounded-lg shadow hover:shadow-md transition-all ${
                        loading[source.id] ? 'opacity-75 cursor-not-allowed' : ''
                      }`}
                    >
