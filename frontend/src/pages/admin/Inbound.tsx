...
const Inbound: React.FC = () => {
  ...
  const { power_bi_id, powerBiUsers, isLoadingPowerBiUsers, powerBiError } = useSelector((state: RootState) => state.item);
  const [loading, setLoading] = useState<Record<number, boolean>>({});
  const [searchParams, setSearchParams] = useSearchParams();
  const [isAuthWindowOpen, setIsAuthWindowOpen] = useState(false);
  const [activeAuthWindow, setActiveAuthWindow] = useState<Window | null>(null);
  ...
  useEffect(() => {
    ...
    // Restore power_bi_id from localStorage on initial load
    const storedPowerBiId = localStorage.getItem("power_bi_id");
    if (storedPowerBiId && !power_bi_id) {
      dispatch(setPowerBiId(storedPowerBiId));
    }
  }, [dispatch, power_bi_id]);
  ...
  const fetchPowerBiUsers = async () => {
    dispatch(setIsLoadingPowerBiUsers(true));
    try {
      const response = await fetch("/api/power-bi-users", {
        method: "POST",
        headers: {
      ...
      dispatch(setPowerBiError(null));
    } catch (error) {
      console.error("Error fetching Power BI users:", error);
      dispatch(setPowerBiError("Failed to load Power BI connections"));
      toast.error("Failed to load Power BI connections", {
        position: "top-right",
        autoClose: 5000,
      });
    } finally {
      dispatch(setIsLoadingPowerBiUsers(false));
    }
  };
  ...
  const handleRefreshUsers = async () => {
    ...
    try {
      ...
      console.error("Error refreshing Power BI users:", error);
      dispatch(setPowerBiError("Failed to refresh connections"));
      toast.error("Failed to refresh connections", {
        position: "top-right",
        autoClose: 5000,
      });
  ...
  const handleSelectPowerBiUser = (powerBiId: string) => {
    dispatch(setPowerBiId(powerBiId));
    toast.success("Selected Power BI connection updated", {
      position: "top-right",
      autoClose: 3000,
    });
  };
  ...
  const handleDeleteConnection = async (email: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Remove the deleted user from the Redux store
        const updatedUsers = powerBiUsers.filter(user => user.power_bi_email !== email);
        dispatch(setPowerBiUsers(updatedUsers));
        // Clear power_bi_id if the deleted user was selected
        if (powerBiUsers.find(user => user.power_bi_email === email)?.power_bi_id === power_bi_id) {
          dispatch(setPowerBiId(null));
        }
      } else if (response.status === 404) {
        toast.error("Connection not found", {
          position: "top-right",
      ...
      console.error("Error deleting Power BI connection:", error);
      dispatch(setPowerBiError(error.message || "Failed to remove connection"));
      toast.error(error.message || "Failed to remove connection", {
        position: "top-right",
        autoClose: 5000,
      });
  ...
  const handleAuthClick = async (source: typeof dataSources[0]) => {
    ...
    try {
      ...
      console.error("Authentication error:", error);
      dispatch(setPowerBiError("Failed to initiate authentication. Please try again."));
      toast.error("Failed to initiate authentication. Please try again.", {
        icon: "❌",
        position: "top-right",
        autoClose: 5000,
                        ...
                        {expandedPowerBi && (
                          ...
                          >
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <button
                                onClick={handleRefreshUsers}
                                disabled={isLoadingPowerBiUsers}
                                className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 disabled:text-gray-400 transition-colors"
                              >
                                <RefreshCw className={`w-3 h-3 ${isLoadingPowerBiUsers ? "animate-spin" : ""}`} />
                                Refresh
                              </button>
                            </div>
                            <div className="flex gap-4 mb-4">
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2">
                                      {user.connection_status === 'Active' && (
                                        <button
                                          onClick={() => handleSelectPowerBiUser(user.power_bi_id)}
                                        ...
                                        >
                                          {power_bi_id === user.power_bi_id ? 'Selected' : 'Select'}
                                        </button>
                                      )}
                                      <button
                                        onClick={() => handleDeleteConnection(user.power_bi_email)}
