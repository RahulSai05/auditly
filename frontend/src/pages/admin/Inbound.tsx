...
const Inbound: React.FC = () => {
  ...
  const handleActivateConnection = async (email: string) => {
    if (!userId) {
      toast.error("User not authenticated. Please login again.", {
        position: "top-right",
        autoClose: 5000,
    ...
    try {
      // Placeholder: Replace with actual API call to activate the connection
      // Example: await dispatch(activatePowerBiUser({ email, userId })).unwrap();
      console.log(`Activating connection for email: ${email}`);
      toast.success(`Connection for ${email} activated successfully`, {
        position: "top-right",
        autoClose: 3000,
      });
      // Refresh the users list after activation
      await dispatch(fetchPowerBiUsers({ userId, connectionType: "inbound" })).unwrap();
    } catch (error: any) {
      console.error("Error activating Power BI connection:", error);
      ...
      <div className={`min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 relative ${isAuthWindowOpen ? "pointer-events-none" : ""}`}>
        ...
        <div className={`max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8 transition-opacity duration-200 ${isAuthWindowOpen ? "opacity-50" : "opacity-100"}`}>
          ...
          {dataSources.length === 0 ? (
            ...
            >
              {dataSources.map((source) => {
                console.log(`Rendering card for source: ${source.title} (id: ${source.id}), expandedPowerBi: ${expandedPowerBi}`);
                return (
                  <motion.div
                    key={source.id}
                    variants={itemVariants}
                    whileHover={{ y: -8, transition: { type: "spring", stiffness: 300 } }}
                    className="bg-white/80 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border border-blue-50 min-h-[200px]"
                  >
                    <div
                      className="h-2"
                      style={{ background: `linear-gradient(to right, ${source.color}40, ${source.color}60)` }}
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        {/* Expansion toggle button only for Power BI */}
                        {source.id === 1 && (
                          <button
                            onClick={togglePowerBiExpand}
                            className="text-gray-500 hover:text-gray-700 transition-colors"
                      ...
                      {source.id === 1 ? (
                        <div className="relative">
                          ...
                          {/* Expanded content only for Power BI when expandedPowerBi is true */}
                          {expandedPowerBi && (
                            <motion.div
                              initial={{ opacity: 0, height: 0 }}
                              animate={{ opacity: 1, height: "auto" }}
                                  ...
                                  {(activeTab === "active" ? activeConnections : inactiveConnections).map((user, index) => (
                                    ...
                                    >
                                      ...
                                      <div className="flex items-center gap-2">
                                        {/* Activate button for inactive connections */}
                                        {activeTab === "inactive" && (
                                          <button
                                            onClick={() => handleActivateConnection(user.power_bi_email)}
                                            className="p-1.5 rounded-full text-green-500 hover:bg-green-50 hover:text-green-600 transition-colors"
                                            title="Activate connection"
                                          >
                                            <Check className="w-4 h-4" />
                                          </button>
                                        )}
                                        {/* Delete button for all connections */}
                                        <button
                                          onClick={() => handleDeleteConnection(user.power_bi_email)}
                                          className="p-1.5 rounded-full text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors"
                                          title="Remove connection"
