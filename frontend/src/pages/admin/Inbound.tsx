...
const Inbound: React.FC = () => {
  ...
  const togglePowerBiExpand = () => {
    console.log("Toggling Power BI expansion, current state:", expandedPowerBi);
    setExpandedPowerBi(!expandedPowerBi);
  };
      ...
      <div className={`min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 relative ${isAuthWindowOpen ? "pointer-events-none" : ""}`}>
        ...
        <div className={`max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8 transition-opacity duration-200 ${isAuthWindowOpen ? "opacity-50" : "opacity-100"}`}>
          ...
          {dataSources.length === 0 ? (
            ...
            >
              {dataSources.map((source) => {
                console.log(`Rendering card for ${source.title}, expandedPowerBi: ${expandedPowerBi}`);
                return (
                  <motion.div
                    key={source.id}
                    variants={itemVariants}
                    whileHover={{ y: -8, transition: { type: "spring", stiffness: 300 } }}
                    className="bg-white/80 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border border-blue-50 min-h-[200px]" // Added min-h to stabilize card height
                  >
                    <div
                      className="h-2"
                      style={{ background: `linear-gradient(to right, ${source.color}40, ${source.color}60)` }}
                    ...
                    <div className="p-4 flex flex-col h-full">
                      ...
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        {/* Chevron button only for Power BI */}
                        {source.id === 1 && (
                          <button
                            onClick={togglePowerBiExpand}
                            className="text-gray-500 hover:text-gray-700 transition-colors"
                      ...
                      <p className="text-gray-600 text-sm mb-4 flex-grow">{source.description}</p>

                      {/* Expansion section only for Power BI */}
                      {source.id === 1 && expandedPowerBi && (
                        <motion.div
                          key={`expansion-${source.id}`} // Unique key to isolate animation
                          initial={{ opacity: 0, height: 0 }}
                          animate={{ opacity: 1, height: "auto" }}
                          exit={{ opacity: 0, height: 0 }}
                          transition={{ duration: 0.3 }}
                        ...
                        >
                          ...
                          {isLoadingUsers ? (
                            ...
                            <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                              {(activeTab === "active" ? activeConnections : inactiveConnections).map(
                                (user, index) => (
                                  <motion.div
                                    key={index}
                                    initial={{ opacity: 0, y: 10 }}
                      ...
                      <div className="flex justify-between items-center mt-4 pt-2">
                        ...
                        {source.id === 3 ? (
                          ...
                          source.authEndpoint && (
                            ...
                            >
                              {source.id === 1 && powerBiUsers.length > 0 ? "Add Account" : "Connect"}
                            </motion.button>
                          )
                        )}
                      </div>
