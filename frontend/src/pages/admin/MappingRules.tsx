...
const MappingRules: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchColumns = async () => {
      ...
      setError("");
      try {
        const response = await fetch(`/columns/${selectedTable}`);
        ...
        setError("Failed to fetch columns for the selected table.");
      } finally {
  ...
  const handleSaveMapping = async () => {
    ...
    try {
      ...
      console.log(`Saved mapping for ${selectedTable}:`, mappingRows);
      setShowSuccess(true);
      setEditMode(false);
      setHasChanges(false);
      ...
      setError("Failed to save mapping rule. Please try again.");
    } finally {
        ...
        >
          ...
          <div className="p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-semibold text-gray-800">Table Mapping</h2>
            </div>
            ...
            {loading.columns ? (
              ...
              <motion.div variants={itemVariants}>
                <table className="w-full">
                  <thead>
                    <tr className="bg-blue-50/50">
                      <th className="p-3 text-left text-gray-700">Source Columns</th>
                      <th className="p-3 text-left text-gray-700">Destination Mapping</th>
                    </tr>
                  </thead>
                  <tbody>
                    {mappingRows.map((row, index) => (
                      <tr
                        ...
                        className="border-b border-blue-100 hover:bg-blue-50/30 transition-colors duration-200"
                      >
                        <td className="p-3 text-gray-700">{row.source}</td>
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <motion.input
                              type="text"
                              ...
                              onFocus={() => setFocusedField(`destination-${index}`)}
                              onBlur={() => setFocusedField(null)}
                              placeholder="Enter destination mapping"
                              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                              disabled={!editMode}
                              animate={
                            ...
                            {editMode && index < mappingRows.length - 1 && (
                              <button
                                onClick={() => swapRows(index, index + 1)}
                                className="text-blue-600 hover:text-blue-800 p-1 hover:bg-blue-50 rounded"
                              >
                ...
                <div className="flex justify-end mt-4 gap-2">
                  <motion.button
                    ...
                    whileHover="hover"
                    whileTap="tap"
                    disabled={loading.saving || !selectedTable}
                  >
                    <Edit className="w-4 h-4" />
                    Edit
                  </motion.button>
                  <motion.button
                    ...
                    whileHover="hover"
                    whileTap="tap"
                    disabled={loading.saving || !editMode || !hasChanges}
                  >
                    {loading.saving ? (
                      <motion.span 
                        className="flex items-center gap-2"
