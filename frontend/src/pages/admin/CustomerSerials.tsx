...
const CustomerSerials: React.FC = () => {
  ...
  const filteredSalesData = useMemo(() => {
    return customerData.filter((data) => {
      // Check if the data matches all the filter criteria
      ...
      if (searchFilters.dateRange.start && searchFilters.dateRange.end) {
        const startDate = parseISO(searchFilters.dateRange.start);
        const endDate = parseISO(searchFilters.dateRange.end);
        const dateToCheck = parseISO(data[`date_${searchFilters.dateType}`]);

        matchesDateRange = isWithinInterval(dateToCheck, { start: startDate, end: endDate });
      }
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="px-4 py-2 bg-blue-100 text-blue-600 rounded-xl hover:bg-blue-200 transition-colors flex items-center gap-2"
                    >
                ...
                <div className="relative mb-6">
                  <div className="flex gap-4">
                    <div className="relative flex-1">
                      ...
                      <input
                        type="text"
                        placeholder="Quick search by sales order..."
                        className="w-full pl-10 pr-4 py-3 bg-white/50 backdrop-blur-sm border-2 border-blue-100 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-300 transition-all duration-300 text-base shadow-sm"
