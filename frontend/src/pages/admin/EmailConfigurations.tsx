...
//           item.id === id ? { 
//             ...item,
//             ...data // Use the response data directly
//           } : item
//         )
//       );
...
function EmailConfigurations() {
  ...
  const handleUpdate = async (id: string) => {
    try {
      ...
      await axios.put(`/api/team-emails/${id}`, payload);
      
      // Update the local state with the form data immediately
      setTeamEmails(prev => 
        prev.map(item => 
          item.id === id ? { 
            ...item,
            email: editForm.email || item.email,
            description: editForm.description || item.description,
            team_name: editForm.team_name || item.team_name
          } : item
        )
      );
