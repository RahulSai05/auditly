...
const AuditlyInspection = () => {
  ...
  const exportToPDF = async () => {
    ...
    const pdfPromises = Array.from(selectedItems).map(async (receiptNumber) => {
      ...
      const imageWidth = 80;
      const images = [
        item.images?.difference_images?.front && { url: item.images.difference_images.front, title: 'Front Difference Image', score: item.images.similarity_scores.front },
        item.images?.difference_images?.back && { url: item.images.difference_images.back, title: 'Back Difference Image', score: item.images.similarity_scores.back },
      ...
      for (const image of images) {
        if (image && image.url) {
          try {
            ...
            if (yPos + 60 > pdf.internal.pageSize.getHeight() - margin) {
              pdf.addPage();
              yPos = margin;
            }

            pdf.setFontSize(14);
            pdf.text(image.title, margin, yPos);
            yPos += 5;

            pdf.addImage(image.url, 'JPEG', margin, yPos, imageWidth, 0); // Height auto-calculated
            yPos += 50;

            if (image.score) {
              pdf.setFontSize(10);
              pdf.text(`Similarity Score: ${image.score}`, margin, yPos);
              yPos += 10;
            }
          } catch (error) {
            pdf.setFontSize(10);
            pdf.setTextColor(255, 0, 0);
            pdf.text(`Failed to load ${image.title}`, margin, yPos);
            pdf.setTextColor(0, 0, 0);
            yPos += 10;
          }
        }
