...
const AuditlyInspection = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      ...
      return matchesReturnOrder && matchesItemDescription && matchesProductCondition;
    });
  }, [data, searchFilters]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <div className="bg-white rounded-xl p-6 text-center">
              <div className="text-red-500 mb-4">
                ...
                <div className="text-sm text-gray-500 mt-2">
                  Searching for receipt: {searchFilters.receiptNumber || '(none)'}
                </div>
              </div>
              <button
                onClick={() => window.location.reload()}
            ...
            <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-xl border border-blue-50 overflow-hidden">
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                      ...
                      <table className="min-w-full divide-y divide-blue-100">
                        ...
                        <tbody className="divide-y divide-blue-50">
                          {filteredData.length === 0 ? (
                            ...
                            filteredData.map((item, index) => (
                              <tr key={`${item.receipt_number}-${index}`} className="hover:bg-blue-50/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600">
                                  {item.receipt_number || 'N/A'}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
