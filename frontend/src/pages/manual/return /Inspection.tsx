...
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../../../store/store";
import { setinspectionData, setCustomerId, selectInspectionData } from "../../../store/slices/itemSlice";
import { Package, Shield, Droplets, AlertCircle, ChevronRight, CheckCircle2, ArrowLeft } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import axios from "axios";
...
export default function Inspection() {
  ...
  useEffect(() => {
    // Initialize form from Redux store if data exists
    if (inspectionData) {
      const newCondition: string[] = [];
      if (inspectionData.factory_seal) newCondition.push("factory_seal");
      if (inspectionData.no_factory_seal) newCondition.push("no_factory_seal");
      if (inspectionData.minimal_tear) newCondition.push("minimal_tear");
      if (inspectionData.no_package) newCondition.push("no_package");
      setCondition(newCondition);

      const newProductCondition: string[] = [];
      if (inspectionData.new_conditiono) newProductCondition.push("new_condition");
      if (inspectionData.not_new_condition) newProductCondition.push("not_new_condition");
      setProductCondition(newProductCondition);

      const newIssue: string[] = [];
      if (inspectionData.bio_stains) newIssue.push("bio_stains");
      if (inspectionData.package_stains) newIssue.push("package_stains");
      setIssue(newIssue);
    }
  }, [inspectionData]);
  ...
  const handleNext = async () => {
    ...
    const inspectionPayload = {
      ...
      minimal_tear: condition.includes("minimal_tear"),
      no_package: condition.includes("no_package"),
      new_conditiono: productCondition.includes("new_condition"),
      not_new_condition: productCondition.includes("not_new_condition"),
      bio_stains: issue.includes("bio_stains"),
      package_stains: issue.includes("package_stains"),
    };
    ...
    try {
      dispatch(setinspectionData(inspectionPayload));
      await new Promise(resolve => setTimeout(resolve, 500));
      navigate("/return/upload-media");
    } catch (error) {
      console.error("Error navigating to media upload:", error);
      setErrors({ navigation: "Failed to proceed. Please try again." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const CheckboxCard = ({
    ...
    <motion.label
      ...
      className={`
        relative flex items-center p-4 cursor-pointer rounded-xl
        transition-all duration-200 
        ${checked ? "bg-blue-50 border-blue-200 shadow-md" : "bg-white border-gray-100 hover:border-blue-100 hover:bg-gray-50"}
        border-2
      `}
    >
      <input
  ...
  const SectionTab = ({
    ...
    <motion.button
      ...
      className={`
        flex items-center gap-3 px-6 py-4 rounded-lg transition-all duration-200 relative
        ${active ? "bg-blue-50 text-blue-700 shadow-sm" : "text-gray-600 hover:bg-gray-50"}
      `}
    >
      {icon}
      <span className="font-medium">{label}</span>
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto p-6">
        ...
        <motion.button
          ...
          className="mt-4 flex items-center gap-2 px-6 py-3 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-all duration-200 shadow-sm"
        >
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto p-6">
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="mt-8 mb-6">
        <ProductDetails />
      </motion.div>

      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl shadow-sm border mt-8">
        <div className="flex gap-2 p-4 border-b overflow-x-auto">
          <SectionTab id="package" label="Package Condition" icon={<Package className="w-5 h-5" />} active={activeSection === "package"} hasError={!!errors.condition} />
          <SectionTab id="product" label="Product Condition" icon={<Shield className="w-5 h-5" />} active={activeSection === "product"} hasError={!!errors.productCondition} />
          <SectionTab id="issues" label="Surface Issues" icon={<Droplets className="w-5 h-5" />} active={activeSection === "issues"} hasError={!!errors.issue} />
        </div>

        <div className="p-6">
          <AnimatePresence mode="wait">
            {activeSection === "package" && (
              <motion.div key="package" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {conditions.map((item) => (
                  <CheckboxCard key={item.id} item={item} checked={condition.includes(item.label)} onChange={(e) => handleSelectionChange(e, setCondition)} name="condition" />
                ))}
              </motion.div>
            )}

            {activeSection === "product" && (
              <motion.div key="product" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {productConditions.map((item) => (
                  <CheckboxCard key={item.id} item={item} checked={productCondition.includes(item.label)} onChange={(e) => handleSelectionChange(e, setProductCondition)} name="productCondition" />
                ))}
              </motion.div>
            )}

            {activeSection === "issues" && (
              <motion.div key="issues" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {issues.map((item) => (
                  <CheckboxCard key={item.id} item={item} checked={issue.includes(item.label)} onChange={(e) => handleSelectionChange(e, setIssue)} name="issue" />
                ))}
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {Object.keys(errors).length > 0 && (
        <motion.div ref={errorRef} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 p-4 bg-red-50 border border-red-200 text-red-600 rounded-lg">
          <ul className="list-disc list-inside">
            {Object.values(errors).map((error, index) => (
              <li key={index}>{error}</li>
            ))}
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="mt-8 flex justify-end gap-4">
        <motion.button
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate("/return/details")}
