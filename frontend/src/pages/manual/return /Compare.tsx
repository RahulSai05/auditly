...
export default function UploadMedia() {
  ...
  useEffect(() => {
    const fetchComparisonResult = async () => {
      const itemId = localStorage.getItem("lastItemId"); // Retrieve item_id from local storage
      const customerId = localStorage.getItem("lastCustomerId");
      if (!itemId || !customerId) {
        alert(
          "Item ID is not available. Please go back and fetch item details again."
        );
        return;
      }
      try {
        const response = await axios.post(
          "http://54.210.159.220:8000/compare-images/",
          {
            ...
            item_id: parseInt(itemId), // Use retrieved item_id
          },
          {
            headers: { "Content-Type": "application/json" },
          }
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium">
                  {data.front_similarity.toFixed(4)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">SSI</span>
                <span className="text-sm font-medium">
                  {data.ssi_front.toFixed(4)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 bg-gray-50 border p-4 rounded-md">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Cosine Similarity</span>
                <span className="text-sm font-medium">
                  {data.back_similarity.toFixed(4)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">SSI</span>
                <span className="text-sm font-medium">
                  {data.ssi_back.toFixed(4)}
                </span>
              </div>
            </div>
          </div>
