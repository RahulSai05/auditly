...
const Login = () => {
  ...
  // ✅ FIXED: compute roles directly from userData (avoid stale state)
  useEffect(() => {
    if (!userData) return;

    // ✅ If user is not active, always push them to access denied
    if (userData.is_active === false) {
      navigate("/access-denied", { replace: true });
      return;
    }

    const userTypeArray = Array.isArray(userData["User Type"])
      ? userData["User Type"]
      : [];

    const isAdmin = userTypeArray.includes("admin");
    const isReportUser = userTypeArray.includes("reports_user");
    const isInspectionUser = userTypeArray.includes("inspection_user");

    // Redirect based on the user role (your existing logic)
    if (isAdmin && isInspectionUser && isReportUser) {
      navigate("/", { replace: true });
    } else if (isAdmin) {
      navigate("/admin/settings/connectors/inbound", { replace: true });
    } else if (isInspectionUser) {
      navigate("/", { replace: true });
    } else if (isReportUser) {
      navigate("/admin/reports/items", { replace: true });
    } else {
    ...
    try {
      if (!otpSent) {
        ...
        if (data.message === "Login Successfull") {
          ...
          // ✅ NEW: if user is not active, redirect to denied page
          // (Assumes backend sends: is_active: true/false)
          if (userData.is_active === false) {
            localStorage.setItem("token", JSON.stringify(userData));
          ...
          // ✅ Store everything as you already do
          localStorage.setItem("token", JSON.stringify(userData));
          localStorage.setItem("userType", userData.user_type);
          localStorage.setItem("userId", userData["User ID"]);
          localStorage.setItem("approved_agent_id", userData.approved_agent_id || "");
          ...
          // ✅ NEW: store active flag too
          localStorage.setItem("is_active", String(userData.is_active ?? true));
