...
export function Navbar() {
  ...
  // [fetchNotifications and markAsRead functions remain the same]
              ...
              <ul className="lg:flex lg:space-x-6 lg:space-y-0 space-y-4 lg:items-center">
                {isInspectionUser && (
                  <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Link to="/" className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition" onClick={() => setIsOpen(false)}>
                ...
                {isAdmin && (
                  <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Link to="/admin/settings/connectors/inbound" className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition" onClick={() => setIsOpen(false)}>
                ...
                {isReportUser && (
                  <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Link to="/admin/reports/items" className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition" onClick={() => setIsOpen(false)}>
                ...
                {isInspectionUser && (
                  <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Link to="/admin/settings/item-master-upload" className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition" onClick={() => setIsOpen(false)}>
                      <Construction className="w-5 h-5" /> Maintenance
                    </Link>
                  </motion.li>
                )}

                <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link to="/help-center" className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition" onClick={() => setIsOpen(false)}>
                    <MessageCircleQuestion className="w-5 h-5" /> Help Center
                  </Link>
                </motion.li>

                {/* Notification Bell - Moved here */}
                {userData?.id && (
                  <motion.li whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="relative">
                    <button onClick={toggleNotifications} className="hover:text-blue-600 flex items-center gap-x-3 text-sm md:text-base transition p-2 relative">
                      ...
                      {showNotifications && (
                        ...
                        >
                          ...
                          <div className="px-4 py-2 border-t border-gray-200 bg-gray-50 text-center">
                            <button onClick={() => setShowNotifications(false)} className="text-xs text-blue-600 hover:text-blue-800">
                              Close
                            </button>
                          </div>
                        </motion.div>
